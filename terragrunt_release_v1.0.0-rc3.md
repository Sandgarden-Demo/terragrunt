...
```json
{
  ...
  "🛠️ Breaking Changes": [
    "Terragrunt now fails fast when a single directory contains both a standard configuration file and a stack configuration file, so projects must choose one format per folder."
  ],
  "🧹 Chores": [
