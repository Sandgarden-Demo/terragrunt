...
* Improved Windows path handling so HCL path functions and related file resolution behavior now use operating system native paths more consistently. Teams that run Terragrunt on Windows should verify any custom path normalization because path separators may differ from earlier release candidates.

* Refined backend initialization when `disable_init = true` so Terragrunt avoids incorrect backend init behavior in configurations that intentionally disable initialization.
...
* Corrected retryable error matching so Terragrunt evaluates actual process errors more accurately instead of matching unrelated flag text such as `-timeout`.

* Enhanced variable value interpolation so Terragrunt resolves variable driven configuration more reliably in affected setups.

* Stabilized worker pool error reporting so Terragrunt no longer surfaces duplicate errors for the same failure path.

* Improved offline detection for discovery requests so transport and DNS failures now behave as offline scenarios instead of producing less helpful outcomes.
...
* Clarified hook failure messages so hook execution errors distinguish process startup failures from commands that ran and returned a non zero exit code.
